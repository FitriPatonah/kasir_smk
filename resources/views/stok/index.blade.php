...
<div class="layout">
    <aside class="sidebar">
        <div class="brand" style="{{ $role === 'admin' ? 'height:84px;padding:22px 24px;gap:12px;' : 'padding:2px 10px 25px;gap:11px;' }}"><span class="brand-mark">🛒</span><div><strong>{{ $namaToko }}</strong><small>{{ $role === 'admin' ? 'Admin Utama' : 'Kasir' }}</small></div></div>
        <nav class="nav">
            ...
            @if($role === 'admin')
                <a href="{{ $role === 'admin' ? route('admin.laporan') : route('kasir.laporan') }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 20V10"/><path d="M12 20V4"/><path d="M6 20v-6"/></svg></span><span>Laporan</span></a>
            @endif
            @if($role === 'admin')
                <a href="{{ route('admin.riwayat.transaksi') }}"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2h9l3 3v17H6z"/><path d="M9 8h6M9 12h6M9 16h4"/></svg></span><span>Transaksi</span></a>
