...
document.addEventListener('DOMContentLoaded', function(){
    ...
    document.addEventListener('click', function(event){
        if (document.body.classList.contains('editing-product') && !event.target.closest('.edit-form') && !event.target.closest('.btn-edit') && !event.target.closest('.btn-link')) closeEdit();
    });
        ...
        <section class="panel">
            ...
            @forelse($produk as $item)
                <article class="admin-product-card" data-kategori="{{ $item->kategori }}">
                    ...
                    <span class="admin-product-stock">Stok: {{ $item->stok }} | Pajak: {{ $item->pajak ?? 0 }}%</span>
                    <div class="card-actions"><button class="btn-link" type="button" onclick="toggleEdit({{ $item->id }})">Edit</button><form method="POST" action="{{ route('admin.produk.destroy',$item->id) }}" onsubmit="return confirm('Hapus produk {{ addslashes($item->nama_produk) }}?')" style="display:inline-flex;align-items:center;margin:0;">@csrf @method('DELETE')<button class="btn-danger" type="submit" title="Hapus produk" aria-label="Hapus produk" style="border:0;background:transparent;padding:0;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;"><svg class="trash-icon" viewBox="0 0 24 24" fill="none" stroke="#f04444" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7l1-3h4l1 3"/></svg></button></form></div>
                    <form id="edit-{{ $item->id }}" class="edit-form" method="POST" action="{{ route('admin.produk.update',$item->id) }}" enctype="multipart/form-data">@csrf @method('PUT')<div class="field"><label>Barcode</label><input name="barcode" value="{{ $item->barcode }}" required></div><div class="field"><label>Nama Produk</label><input name="nama_produk" value="{{ $item->nama_produk }}" required></div><div class="field"><label>Harga</label><input type="number" name="harga" value="{{ $item->harga }}" required></div><div class="field"><label>Stok</label><input type="number" name="stok" value="{{ $item->stok }}" required></div><div class="field"><label>Pajak (%)</label><input type="number" name="pajak" min="0" max="100" value="{{ $item->pajak ?? 0 }}" required></div><div class="field file-field"><label>Ganti Foto</label><input type="file" name="foto" accept="image/*"></div><button class="btn btn-primary" type="submit">Simpan</button></form>
                </article>
            @empty
                <div class="empty">Belum ada produk.</div>
            @endforelse
            </div>
            <div class="table-wrap"><table><thead><tr><th>#</th><th>Foto</th><th>Barcode</th><th>Nama Produk</th><th>Harga</th><th>Stok</th><th>Pajak</th><th>Aksi</th></tr></thead><tbody>
            @forelse($produk as $i => $item)
                <tr><td>{{ $i+1 }}</td><td>@if($item->foto)<img class="thumb" src="{{ asset('storage/'.$item->foto) }}" alt="{{ $item->nama_produk }}">@else<span class="no-photo">Tanpa foto</span>@endif</td><td>{{ $item->barcode }}</td><td><strong>{{ $item->nama_produk }}</strong></td><td>Rp {{ number_format($item->harga,0,',','.') }}</td><td>@if($item->stok <= 0)<span class="badge badge-out">Habis</span>@elseif($item->stok <= 7)<span class="badge badge-low">{{ $item->stok }}</span>@else<span class="badge badge-ok">{{ $item->stok }}</span>@endif</td><td>{{ $item->pajak ?? 0 }}%</td><td><div class="card-actions"><button class="btn-link" type="button" onclick="toggleEdit({{ $item->id }})">Edit</button><form method="POST" action="{{ route('admin.produk.destroy',$item->id) }}" onsubmit="return confirm('Hapus produk {{ addslashes($item->nama_produk) }}?')" style="display:inline-flex;align-items:center;margin:0;">@csrf @method('DELETE')<button class="btn-danger" type="submit" title="Hapus produk" aria-label="Hapus produk" style="border:0;background:transparent;padding:0;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;"><svg class="trash-icon" viewBox="0 0 24 24" fill="none" stroke="#f04444" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M6 7l1 13h10l1-13M9 7l1-3h4l1 3"/></svg></button></form></div><form id="edit-{{ $item->id }}" class="edit-form" method="POST" action="{{ route('admin.produk.update',$item->id) }}" enctype="multipart/form-data">@csrf @method('PUT')<div class="field"><label>Barcode</label><input name="barcode" value="{{ $item->barcode }}" required></div><div class="field"><label>Nama Produk</label><input name="nama_produk" value="{{ $item->nama_produk }}" required></div><div class="field"><label>Harga</label><input type="number" name="harga" value="{{ $item->harga }}" required></div><div class="field"><label>Stok</label><input type="number" name="stok" value="{{ $item->stok }}" required></div><div class="field"><label>Pajak (%)</label><input type="number" name="pajak" min="0" max="100" value="{{ $item->pajak ?? 0 }}" required></div><div class="field file-field"><label>Ganti Foto</label><input type="file" name="foto" accept="image/*"></div><button class="btn-primary" type="submit">Simpan</button></form></td></tr>
            @empty
